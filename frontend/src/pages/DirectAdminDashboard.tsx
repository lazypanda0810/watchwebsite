...
const DirectAdminDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setAdminUser({ role: 'admin', firstName: 'Admin' });
    } catch (error) {
      navigate('/admin/login');
      return;
    }
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setStats(prev => ({
          ...prev,
          totalProducts: data.data.pagination?.totalProducts || data.data.products?.length || 0,
          activeProducts: data.data.products?.filter((p: Product) => p.isActive).length || 0
        }));
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center space-x-4">
              <span className="text-gray-700">Welcome, {adminUser?.firstName || 'Admin'}</span>
              <button
      ...
      {/* Product Form Modal - [Previous form code remains the same] */}
      {showForm && (
